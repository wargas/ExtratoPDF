...
namespace ExtratoPDF
{
    ...
    {
        Extrator extractor = Extrator.Factory();
        public MainWindow()
        {
            InitializeComponent();
        ...
        {
            if(!extractor.IsLoaded())
            {
                MessageBox.Show("Erro ao ler a lib de interpretadores", "Atenção");
            }
        }

        private void LoadLibrary()
        {
            statusApp.Text = extractor.GetVersion();

            foreach (var item in extractor.GetBankList())
            {
